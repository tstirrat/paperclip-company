...
**When CI is green — check for collaborator comments, then hand off to human:**

a. Run the `pr-collaborator-comments` skill:
   ```
   bash skills/pr-collaborator-comments/pr-collaborator-comments.sh <owner> <repo> <pr_number>
   ```
   If the output contains collaborator comments: do not proceed. Copy them into the issue, reassign to **CTO**. See `HANDOFFS.md` → "Reviewer or Collaborator Comments".

b. No collaborator comments → mark task `in_review`, comment on the issue with the PR link, and **exit. You are done. Do not merge.**

> "PR ready for review: [PR link]. CI green. Awaiting human approval and merge."

**You never merge.** Merging is a human action. Your job ends when the PR is open, CI is green, and there are no unresolved collaborator comments.
